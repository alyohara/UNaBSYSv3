            ...
            <form method="post" action="{{ route('coordinador.perform') }}">
                ...
                <div class="form-group form-floating mb-3">
                    ...
                    <div class="form-group">
                        ...
                        <select  name="user_id" autofocus id="user_id" required
                                style="width: 100%" class="form-control">
                            <option value="" disabled selected>Seleccione un coordinador</option>

                            @foreach($coords as $coord)
                                <option value="{{ $coord->user->id }}">{{$coord->name.' '.$coord->lastname}}</option>
                            @endforeach
                ...
                <div class="form-group form-floating mb-3">
                    ...
                    <select class="multi" name="career_id[]" autofocus id="career_id[]" multiple="multiple" size="5"
                        ...
                        @foreach($carreras as $career)
                            <option
                                value="{{ $career->id }}">{{$career->CodigoSIU.' - '.$career->DenominacionCarrera}}</option>
                        @endforeach
