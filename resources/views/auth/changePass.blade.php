...
@section('content')
    ...
    <div class="bg-light p-5 rounded">
        @auth
            <form method="post" action="{{ route('register.changePassUpdate') }}">
                <a style="margin-bottom: 5px; float: right" href="#" class="  btn btn-secondary "
                   onclick="window.history.back();">Volver</a>
                <input type="hidden" name="_token" value="{{ csrf_token() }}" />

                <h1 class="h3 mb-3 fw-normal">Modificar Contraseña: <span class="text-info">{{ $user->username }}</span></h1>
...
<input type="hidden" value="{{$user->id}}" name="userid" id="userid">
                ...
                <button class="w-100 btn btn-lg btn-primary" type="submit">Modificar Contraseña</button>
