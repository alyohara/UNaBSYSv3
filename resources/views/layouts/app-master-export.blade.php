...
<script>
    $(document).ready(function () {
        ...
        // Add column filters to specific columns
        ...
        // Clone filter row and append it to footer
        var filterRowFooter = $('#filterRow').clone();
        $('#filterRowFooter').html(filterRowFooter.html());

        // Apply the column filters
        $('#filterRow input').on('keyup change', function() {
            var columnIndex = $(this).parent().index();
            // Exclude columns where you don't want to apply filters
            if ((columnIndex !== 11) && (columnIndex !== 10)) {
                table.column(columnIndex).search(this.value).draw();
            }
        });

        // Apply the column filters in the footer
        $('#filterRowFooter input').on('keyup change', function() {
            var columnIndex = $(this).parent().index();
            // Exclude columns where you don't want to apply filters
            if ((columnIndex !== 11) && (columnIndex !== 10)) {
                table.column(columnIndex).search(this.value).draw();
        ...
        // $('#Docencia').on('click', function () {
        //     if ($(this).is(':checked')) {
        //         table.columns(10).search('Docencia').draw();
        //
        //     }else{
        //         table.columns(10).search('').draw();
        //         if ($('#Extension').is(':checked')) {
        //             table.columns(10).search('Extension').draw();
        //         }
        //         if ($('#Gestion').is(':checked')) {
        //             table.columns(10).search('Gestion').draw();
        //         }
        //         if ($('#Investigacion').is(':checked')) {
        //             table.columns(10).search('Investigacion').draw();
        //         }
        //     }
        // });
        //
        ...
        //     if ($(this).is(':checked')) {
        //         table.columns(10).search('Extension');
        //         if ($('#Docencia').is(':checked')) {
        //             table.columns(10).search('Docencia');
        //         }
        //         if ($('#Gestion').is(':checked')) {
        //             table.columns(10).search('Gestion');
        //         }
        //         if ($('#Investigacion').is(':checked')) {
        //             table.columns(10).search('Investigacion');
        //         }
        //         table.draw();
        //
        //     }else{
        ...
        $('#columnFilter').on('change', 'input[type="checkbox"]', function () {
            ...
            // Combine selected options using OR (|) to create the regex filter
            var filterValue = selectedOptions.join('|');

            // Apply the custom filter
            table.column(10) // Replace '1' with the index of the column you want to filter (0-indexed)
                .search(filterValue, true, false) // true for regex, false for smart search
                .draw();
        });
